...
order_md <- function(INPUT, OUTPUT, CONDITIONAL = NULL, DISCRETE = NULL, M = 25, B = 100, hlevel = .1, LOG = FALSE, ConditionalControl = FALSE, BUGCHECK = FALSE, MovingMFactor = NULL, StandardM = 'NO'){
  ...
  for (i in 1:N){
    ...
    }else if(StandardM == 'INPUT'){
      dataadjusted <- datatemp %>% filter(ylarger == TRUE) 
    }else if (StandardM == 'OUTPUT'){
      dataadjusted <- datatemp %>% filter(xsmaller == TRUE)
    }
    
    compareable <- nrow(dataadjusted)
    if(is.numeric(MovingMFactor) == TRUE){
      Mtemp <- floor(M*compareable^MovingMFactor)
    }
    return_test <- order_md_score(dataadjusted, M = Mtemp, B = B, probdone = probdone)
    return_test <- cbind(return_test, compareable)
    
    end_time <- Sys.time()
    if(BUGCHECK == TRUE){
      print(end_time - start_time)
      print(i)
      return_test <- cbind(return_test, Mtemp)
    }
